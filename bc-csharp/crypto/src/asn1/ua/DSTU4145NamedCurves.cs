...
namespace Org.BouncyCastle.Asn1.UA {
  ...
  public class DSTU4145NamedCurves
  {
      ...
      public static ECDomainParameters[] paramsV = new ECDomainParameters[10];
      static DerObjectIdentifier[] oids = new DerObjectIdentifier[10];

      //All named curves have the following oid format: 1.2.804.2.1.1.1.1.3.1.1.2.X
      //where X is the curve number 0-9
      static string oidBase = UAObjectIdentifiers.dstu4145le.Id + ".2.";
      ...
      static DSTU4145NamedCurves()
      {
          ...
          F2mCurve[] curves = new F2mCurve[10];
          ...
          {
              paramsV[i] = new ECDomainParameters(curves[i], points[i], n_s[i], h_s[i]);
          }

          for (int i = 0; i < oids.Length; i++)
          {
              oids[i] = new DerObjectIdentifier(oidBase + i);
          }
      }
       ...
       * where X is the curve number 0-9
       */
      public static DerObjectIdentifier[] getOIDs()
      {
          return oids;
      }
      ...
      {
          string oidStr = oid.Id;
          if (oidStr.StartsWith(oidBase))
          {
              int index = Int32.Parse(oidStr.Substring(oidStr.Length - 1));
              return paramsV[index];
          }
          return null;
      }
  }
