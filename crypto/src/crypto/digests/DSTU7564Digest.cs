...
{
     /**
	* implementation of Ukrainian DSTU 7564 (Ukrainian national standard of hashing transformation).
     * Thanks to Roman Oleinikov' native C implementation: https://github.com/Roman-Oliynykov/Kupyna-reference
	*/
          ...
          : IDigest
     {
          ...
          {
               padded_ = Pad(input, inOff, length);
                             
               int paddedOff = 0;
               int paddedLength = padded_.Length;
          ...
          protected byte[] Pad(byte[] input, int inOff, int length)
          {
               ...
               {
                    padded = new byte[length + blockSize];
               }
               else
               {
               ...
               padded[length] = 0x80;
               intTobytes(length * 8, padded, padded.Length - 12);
          ...
          public void Reset()
          {
               for (int i = 0; i < state_.Length; i++)
               {
                    state_[i] = new byte[columns];
               }

               state_[0][0] = (byte)state_.Length;

               Array.Clear(padded_, 0, padded_.Length);
